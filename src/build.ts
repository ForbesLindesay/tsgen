...
write(__dirname + '/walkers/index.ts', `
...
export default function walk(node: bt.Statement, ctx: Context): bt.Identifier[] {
  switch (node.type) {
${builders.map(n => `    case '${n}': return walk${n}(node as bt.${n}, ctx);`).join('\n')}
    default:
      writeFileSync(
...
write(__dirname + '/printers/index.ts', `
import {writeFileSync} from 'fs';
import * as bt from 'babel-types';
...
export default function print(node: bt.Node, ctx: Context): string {
  switch (node.type) {
${printers.map(n => `    case '${n}': return print${n}(node as ${special(n) ? '' : 'bt.'}${n}, ctx);`).join('\n')}
    default:
      writeFileSync(
        __dirname + '/' + node.type + '.ts',
        "import * as bt from 'babel-types';\\n" +
